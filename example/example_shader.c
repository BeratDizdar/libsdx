...
int game_loop(float dt) {
    ...
    sdx_SetVertexDeclaration(dev, v_decl);
    sdx_SetVertexShader(dev, v_shader);
    sdx_SetPixelShader(dev, p_shader);
    sdx_SetOrtho(dev, 0, 800.0f, 600.0f);
...
int main() {
    swl_CreateWindow("Shader Demo", 800, 600);
    sdx_CreateContext(&ctx);
    sdx_CreateDevice(ctx, swl_GetWindowPtr(), &dev);
    int layout[] = { SDX_LAYOUT_POS2, SDX_LAYOUT_UV2, SDX_LAYOUT_END };
    sdx_CreateVertexDeclaration(dev, layout, &v_decl);
    sdx_CreateVertexShader(dev, (unsigned long*)g_vs20_VS, &v_shader);
    sdx_CreatePixelShader(dev, (unsigned long*)g_ps20_PS, &p_shader);
    sdx_CreateTextureFromPath(dev, "t.png", &tex);
    sdx_CreateVertexBuffer(dev, sizeof(Vertex) * 4, 0, 0, 0, &vb);
    
    swl_Mainloop(game_loop);
    
    sdx_ReleaseArgs(vb, tex, v_shader, p_shader, v_decl, dev, ctx, NULL);
    swl_CloseWindow();
